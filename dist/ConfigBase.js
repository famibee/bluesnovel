...
var import_platform = (/* @__PURE__ */ __commonJSMin(((e, t) => {
	(function() {
		var n = {
			function: !0,
			object: !0
...
var ConfigBase = class {
	...
	async load(e) {
		...
		let t = {};
		if (this.sys.arg.crypto) for (let [e, n] of Object.entries(this.hPathFn2Exts)) for (let [r, i] of Object.entries(n)) {
			if (!r.startsWith(":")) {
				t[e] = r;
				continue;
			}
			if (!r.endsWith(":id")) continue;
			let a = i.slice(i.lastIndexOf("/") + 1), o = n[r.slice(0, -10)] ?? "", s = await (await this.sys.fetch(o)).text();
			if (a !== this.sys.hash(s)) throw `ファイル改竄エラーです fn:${o}`;
		}
		else for (let [e, n] of Object.entries(this.hPathFn2Exts)) for (let r of Object.keys(n)) r.startsWith(":") || (t[e] = r);
	}
	#e = !1;
	get existsBreakline() {
		return this.#e;
	...
	searchPath(e, t = SEARCH_PATH_ARG_EXT.DEFAULT) {
		...
		if (this.userFnTail) {
			...
			if (e in this.hPathFn2Exts) {
				if (t === "") i = e;
				else for (let n of Object.keys(this.hPathFn2Exts[e] ?? {})) if (`|${t}|`.includes(`|${n}|`)) {
					i = e;
					break;
				}
			}
		...
		if (!a) {
			let r = int(o[":cnt"]);
			if (t === "") {
				if (r > 1) throw `指定ファイル【${e}】が複数マッチします。サーチ対象拡張子群【${t}】で絞り込むか、ファイル名を個別にして下さい。`;
				return e;
			}
			let i = `|${t}|`;
			if (r > 1) {
				let n = 0;
				for (let r of Object.keys(o)) if (i.includes(`|${r}|`) && ++n > 1) throw `指定ファイル【${e}】が複数マッチします。サーチ対象拡張子群【${t}】で絞り込むか、ファイル名を個別にして下さい。`;
			}
			for (let e of Object.keys(o)) if (i.includes(`|${e}|`)) return o[e];
			throw `サーチ対象拡張子群【${t}】にマッチするファイルがサーチパスに存在しません。探索ファイル名=【${e}】`;
		}
		if (t !== "" && !`|${t}|`.includes(`|${a}|`)) throw `指定ファイルの拡張子【${a}】は、サーチ対象拡張子群【${t}】にマッチしません。探索ファイル名=【${e}】`;
		let c = o[a];
		if (!c) throw `サーチパスに存在しない拡張子【${a}】です。探索ファイル名=【${e}】、サーチ対象拡張子群【${t}】`;
		return c;
	}
	matchPath(e, t = SEARCH_PATH_ARG_EXT.DEFAULT) {
...
export { uint as a, getFn as i, SEARCH_PATH_ARG_EXT as n, CmnLib as r, ConfigBase as t };
