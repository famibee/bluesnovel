...
var yo = class p {
	...
	constructor(e) {
		this.sys = e, this.#m = new n(e, ""), this.#w = new oe((t, n) => e.cfg.searchPath(t, n), (t, n) => e.fetch(t, n), (t, n) => e.dec(t, n), (t) => e.decAB(t), e.crypto), this.#e = document.createElement("span"), this.#e.hidden = !0, this.#e.textContent = "", this.#e.style.cssText = `	z-index: ${2 ** 53 - 1};
			...
			color: black;
			background-color: rgba(255, 255, 255, 0.7);`, document.body.appendChild(this.#e), this.#t.trace = (e) => this.#rt(e), this.#t.log = (e) => this.#at(e, this.#r?.fn ?? "", this.#r?.lineNum ?? NaN);
	}
	destroy() {
	...
	#y;
	async #b(e) {
		return this.#n[e] ??= new S(e, await this.#nt(e), this.#S());
	}
	#x;
	#S() {
		...
		if (e) {
			...
			try {
				for (;;) {
					...
					if (n?.t !== "loadScript") {
						e.atEnd ? this.myTrace(`スクリプト終端です fn:${e.fn}`, "I") : this.#et();
						return;
					}
					try {
	...
	async #Ue(e) {
		...
		let r = e.fn.startsWith(f), i = r ? e.fn : ae(e.fn || "snapshot"), a = re(i), { stageW: o, stageH: s } = t, c = e.width || o, l = e.height || s, u = (e.aLayNm === null && e.page === "fore" && e.b_color === void 0 ? await this.sys.capturePage(this.#We(n), c, l, a) : "") || await te({
			el: n,
			sw: o,
			sh: s,
			width: c,
	...
	#qe(e) {
		...
		return this.#Ke[e] ??= this.sys.cfg.matchPath(`^${t}$`, d.SP_GSM).length > 0 ? this.sys.cfg.searchPath(t, d.SP_GSM) : "";
	}
	#Je(e, t) {
		if (!t) return "";
	...
	#et() {
		...
		if (e) for (let t of new Set(e.peekUpcomingPicFn())) {
			...
			this.#$e.has(e) || this.#$e.set(e, this.#Xe(e));
		}
	}
	#tt(e) {
		switch (e.t) {
			...
			case "layPlg": break;
			case "chgPic": {
				let t = this.#Je("lay", e.fn), { isSheet: n, isMovie: r } = vo(t), i = e.aFace?.map((e) => {
				...
				let s = (e) => {
					let t = this.#$e.get(e);
					return t && this.#$e.delete(e), t ?? this.#Xe(e);
				};
			...
			case "addFx": {
				...
				let n = `${e.aLayNm?.join(",") ?? ""}:${e.page}:${e.fx.name}`, r = (this.#Qe.get(n) ?? 0) + 1;
				this.#Qe.set(n, r), this.#Xe(t).then((t) => {
					this.#Qe.get(n) === r && this.$fncs.chgFx({
						aLayNm: e.aLayNm,
						page: e.page,
						mode: "add",
						fx: {
				...
				}).catch((t) => this.myTrace(`[add_fx] tex= の復号に失敗しました fn:${e.fx.tex} ${String(t)}`, "E")), this.#pe(e);
				break;
			}
			case "clearFx":
				this.$fncs.chgFx({
			...
			case "trace":
				...
				break;
			case "log":
				this.#at({ text: e.text }, e.fn, e.lineNum);
				break;
