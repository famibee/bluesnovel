...
var co = class p {
	...
	#e;
	constructor(e) {
		this.sys = e, this.#m = new n(e, ""), this.#C = new ne((t, n) => e.cfg.searchPath(t, n), (t, n) => e.fetch(t, n), (t, n) => e.dec(t, n), (t) => e.decAB(t), e.crypto), this.#e = document.createElement("span"), this.#e.hidden = !0, this.#e.textContent = "", this.#e.style.cssText = `	z-index: ${2 ** 53 - 1};
	...
	async #p(e) {
		...
		try {
			...
			this.#d = !0, this.#f(), t.restoreMarkPart(n.mark), t.clearOnResume = n.clearOnResume, this.#ce(null, "both"), this.#he(() => !0), this.$fncs.replace(n.mark.sPages), this.#w.playback(n.mark.hPlgLay, []), this.#M = !1, this.#l = void 0, t.switchScript(await this.#b(n.fn), "", n.idx);
		} catch (e) {
	...
	#y;
	async #b(e) {
		return this.#n[e] ??= new x(e, await this.#at(e), this.#S());
	}
	#x;
	#S() {
	...
	#E = new le((e, t) => this.myTrace(e, t), (e, t) => this.sys.fetch(e, t), (e) => this.sys.decAB(e));
	unlockAudio() {
		this.#E.unlock();
	}
	needClick2Play() {
	...
	#j(e = !1) {
		if (!this.#M) {
			...
			if (this.#Ce) {
				this.#Ce.canskip && this.#Te();
				return;
			}
			if (this.#ke) {
				this.#ke.canskip && this.#je();
				return;
			}
			if (this.#Ne) {
				this.#Ne.canskip && this.#Ie();
				return;
			}
			this.#N || this.#Be().catch(this.#i);
		}
	}
	#M = !1;
	#N = !1;
	...
	#pe(e) {
		...
		let t = ++this.#de, n = setTimeout(() => this.#me(t), e.fx.time);
		this.#ue.push({
			id: t,
			timer: n,
			aLayNm: e.aLayNm,
	...
	#ge(e, t, n) {
		let r = this.#ue.filter((n) => p.#_e(n, e, t));
		if (r.length === 0) {
			setTimeout(() => this.#j(), 0);
			return;
		}
	...
	async #ye(e) {
		...
		n && (this.#r?.setValNochk(`tmp:const.sn.sound.${e.buf}.playing`, !0), await this.#E.play(e.buf, n, e, (e) => {
			this.#r?.setValNochk(`tmp:const.sn.sound.${e}.playing`, !1), e === "VOICE" && this.#be();
		}));
	}
	#be() {
		let e = this.#r;
	...
	#we(e, t, n) {
		if (!this.#E.waitEnd(e, () => {
			this.#Ce?.buf === e && (this.#Ce = void 0, this.#j());
		})) {
			setTimeout(() => this.#j(), 0);
			return;
		}
	...
	#Te() {
		let e = this.#Ce;
		e && (this.#Ce = void 0, this.#E.cancelWaitEnd(e.buf), e.stop && this.#E.stop(e.buf), this.#j());
	}
	#Ee = Object.create(null);
	#De(e) {
	...
	#Oe(e) {
		delete this.#Ee[e], this.#ke?.buf === e && (this.#ke = void 0, setTimeout(() => this.#j(), 0));
	}
	#ke;
	#Ae(e, t) {
		if (!this.#Ee[e]) {
	...
	#Fe(e, t, n, r = 30) {
		...
		if (!i) {
			if (r > 0) {
				requestAnimationFrame(() => this.#Fe(e, t, n, r - 1));
				return;
			}
			this.#j();
			return;
		...
		let a = () => {
			if (this.#Ne?.fn !== e) return;
			let t = this.#Pe(e);
			if (!t || t.loop || t.ended) {
				this.#Ne = void 0, t?.ended && n && this.#Le(t), this.#j();
				return;
			}
		...
		if (e) {
			...
			try {
				for (;;) {
					...
					if (n?.t === "waitFx") {
						this.#ge(n.aLayNm, n.names, n.canskip);
						return;
					}
					if (n?.t === "waitQuake") {
						this.#X(n.canskip);
					...
					if (n?.t === "waitVideo") {
						this.#Fe(n.fn, n.canskip, n.stop);
						return;
					}
					if (n?.t === "playSnd" && n.join) {
						this.#N = !0, this.#Se(n).catch(this.#i);
						return;
					}
					if (n?.t === "addFrame" || n?.t === "letFrame") {
						this.#N = !0, this.#Ue(n).catch(this.#i);
						return;
					}
				...
				this.#Re = !1, this.#ze > 0 && (--this.#ze, this.#j());
			}
		}
	}
	#Ve(e) {
	...
	async #Ue(e) {
		try {
			e.t === "addFrame" ? this.#Ye(await this.#C.add(e.id, e.src, e.sty)) : this.#Ye({ [`const.sn.frm.${e.id}.${e.var_name}`]: this.#C.get(e.id, e.var_name, e.fnc) });
		} catch (t) {
	...
	async #We(e) {
		...
		try {
			...
			if (t.restoreMarkPart(n), this.#xe(t), this.$fncs.replace(n.sPages), this.#w.playback(n.hPlgLay, []), this.#c.clear(), this.#l = void 0, this.#M = !1, e.t === "load" && e.doRec !== !1 && (this.#y = { ...n }), e.t === "load" && e.index !== void 0) {
	...
	async #Ge(e) {
		try {
			e.t === "loadPlugin" ? await this.#Ke(e.fn) : await this.#qe(e);
		} catch (t) {
	...
	async #qe(e) {
		...
		let r = e.fn.startsWith(f), i = r ? e.fn : te(e.fn || "snapshot"), a = T(i), { stageW: o, stageH: s } = t, c = e.width || o, l = e.height || s, u = (e.aLayNm === null && e.page === "fore" && e.b_color === void 0 ? await this.sys.capturePage(this.#Je(n), c, l, a) : "") || await C({
			el: n,
			sw: o,
			sh: s,
			width: c,
	...
	#Ze(e) {
		...
		return this.#Xe[e] ??= this.sys.cfg.matchPath(`^${t}$`, d.SP_GSM).length > 0 ? this.sys.cfg.searchPath(t, d.SP_GSM) : "";
	}
	#Qe(e, t) {
		if (!t) return "";
	...
	#rt() {
		...
		if (e) for (let t of new Set(e.peekUpcomingPicFn())) {
			...
			this.#nt.has(e) || this.#nt.set(e, this.#et(e));
		}
	}
	#it(e) {
		switch (e.t) {
			...
			case "layPlg": break;
			case "chgPic": {
				let t = this.#Qe("lay", e.fn), n = t.endsWith(".json"), r = /\.(?:mp4|webm)$/i.test(t), i = e.aFace?.map((e) => {
				...
				let s = (e) => {
					let t = this.#nt.get(e);
					return t && this.#nt.delete(e), t ?? this.#et(e);
				};
			...
			case "autowc":
				...
				break;
			case "clearLay":
				this.#ce(e.aLayNm, e.page), this.#he((t) => (e.page === "both" || t.page === "both" || t.page === e.page) && p.#_e(t, e.aLayNm, null)), this.$fncs.clearLay({
					aLayNm: e.aLayNm,
					page: e.page
			...
			case "clearFx":
				this.$fncs.chgFx({
					...
					mode: "clear",
					names: e.names
				}), this.#he((t) => (e.page === "both" || t.page === "both" || t.page === e.page) && p.#_e(t, e.aLayNm, e.names));
				break;
			case "moveLay":
				this.$fncs.moveLay({
					nm: e.nm,
			...
			case "stopAllSnd":
				...
				break;
			case "xchgBufSnd":
				this.#Me(e.buf), this.#Me(e.buf2), this.#E.xchgBuf(e.buf, e.buf2);
				break;
			case "duckBgm":
			...
			case "frame":
				this.#Ye(this.#C.frame(e.id, e.sty, e.order, e.disabled));
				break;
			case "setFrame":
