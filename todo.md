...
## 引き継ぎ（2026-08-17セッション終盤）

**縦書きグリフ描画不具合（`CHANGELOG.md` 2026-08-17参照）は未解決のまま持ち越し。** 今セッションで
GSAPをWeb Animations APIへ置き換える大きな修正を入れたが、**修正後もユーザーが`tmp_blues`実機
（`最初から`→手動クリックの新規順行、既読の早送りではない）で同じ症状（DOM/CSS状態は正常なのに
グリフだけ出ない）を再現した**。その場での診断で、今セッションが最有力と見ていた2つの仮説が
**両方とも実測で否定された**：

- 対象span（「─」の対）の`getComputedStyle().transform`は両方とも`"none"`
  ——今回除去したGSAPの`force3D`由来`matrix3d(...)`残留は無い
- `[...document.fonts].map(f=>({family:f.family,status:f.status}))`は
  `ipamjm`/`Source Han Sans CN`とも`status:"loaded"`——フォント読み込みレースでもない

つまり**この会話中に検討したメカニズム（GSAP 3D昇格・フォント読み込みレース）はどちらも今回の
発生の原因ではない**。GSAP→Web Animations API化自体は本家との設計比較・実バグ2件の修正
（`sty4Moveable`の恒等transform除去、ルビ`marginBlockStart`の`cvsScale`未対応）を含め独立した
改善として価値があり、単体テスト・E2Eは全件通過・実装は完了しているが、**当初の目的（描画欠落の
解消）は達成できていない**。

ライブでの追加ナッジ（`willChange`をトグルして強制的にコンポジットレイヤを張り直す）を提案し
ユーザーが実行したが、見た目が直ったかどうかの確認が取れないままセッション終了。次回はここから
再開してよい（同じ`tmp_blues`実機・同じ箇所で再現するなら、まずこのナッジが効くかどうかを見る）。

**次回への提案**：
- アプリ側で制御できる要因（本セッションで比較検討したbluesnovel⇔本家の差分）は出尽くした感触。
  Chromiumの縦書き＋独自フォント（`ipamjm`）＋特定グリフという狭い条件でのSkia/フォント
  シェーピング側の間欠バグという可能性を本格的に検討する段階
- 場当たり的な仮説検証を続けるより、①発生時にその場で診断ログ（フォント状態・transform・
  グリフ矩形等）を自動収集する仕組みを先に作る、②再現条件を極限まで削った最小HTMLを作って
  Chromiumへバグ報告する、のどちらかにまとまった時間を割く方が筋が良さそう
- 診断に使ったスクリーンショット・診断コードはこのセッションの会話ログ参照（再掲すると長いため
  ここには貼らない）
