...
**`ss_000.sn:24`（縦書き）改行位置ズレの本題：解決**。真因はルビ付き文字の`margin-block-start`
補正（「ルビが1つ前の列に重なるのを防ぐ」処理）が列の先頭以外の文字にも無条件に掛かり、
その文字がたまたま列の先頭に来た回だけ列全体が前の列側へ押し出されて大きな隙間になっていた
こと（ユーザー指摘の「【ある道具】や【の、安全】の間に空行が入る」現象そのもの）。文字数
だけを比較しても見えない種類の不具合だった（列内の文字は正しく、列と列の"間"が広がるだけ）。
修正はmargin付与を「実際に列の先頭に来た表示単位」だけに絞る形に変更（`TxtLayer.tsx`の
`applyKinsoku()`）。詳細・実測値・テスト結果はCHANGELOG.md 2026-08-18の該当エントリ参照。

### 実機比較の手順（2026-08-18に確立・スクリプト化済み。今後の類似調査でも再利用できる）

**tmp_blues側**：`bash .claude/scripts/kinsoku_compare.sh`（devサーバー起動済みなら
**これを呼ぶだけ**でよい）。列（`<br>`区切り）ごとのテキストをJSON配列で出力する。

**本家(tmp_esm_uc)側**：`bash .claude/scripts/kinsoku_compare_esm.sh`（`localhost:5174`が
未起動ならスクリプトがエラーで教える。起動コマンドはスクリプト冒頭コメント参照。tmp_esm_uc
はこのリポジトリの外側にあるプロジェクトなので絶対パスで`cd`が要る）。列はDOMの`<br>`でなく
`.sn_ch`要素の`x`座標でグループ化して復元している。**セッション名`esm`
（`playwright-cli -s=esm`）を使うため、tmp_blues側と同時に開いたままでよい**。

両スクリプトとも、詰まったときの背景（座標クリックが要る理由、stable待ちが要る理由）は
スクリプト冒頭コメントに書いてある。

**教訓**：文字数・内容（列の中身）が本家と完全一致していても、列と列の"間隔"（x座標）は
別途実測しないと不具合を見逃す。今回、最初の比較（col1〜col6の文字数・内容一致）だけでは
「解決した」と誤判定した——実際の不具合は列間の隙間という、文字の中身を見るだけでは
気付けない種類のものだった。次回似た調査をする時は、文字数だけでなく`getBoundingClientRect()`
のx/y座標も突き合わせること。
