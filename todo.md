...
- [ ] **音声・動画**（一括で未着手）：`[playbgm]` `[stopbgm]` `[fadebgm]` `[fadeoutbgm]` `[playse]` `[stopse]` `[fadese]` `[fadeoutse]` `[volume]` `[xchgbuf]` `[ws]` `[wb]` `[wf]` `[wl]`、`[wv]`（動画待ち）。`ext_voice.sn`の`voice`系マクロも同じ
  ...
  - [ ] 実装時は本家 `SndBuf.ts` の修正点に注意（本家からの引き継ぎ、2026-07-30）：停止経路は必ず`Howl.unload()`を通す（howlerは`Howler._howls`にインスタンスを持ち続け、`unload()`でしか除去しない。「ロード完了前に停止」した時だけ解放漏れになりやすい）。**`unload()`後は`loaderror`が来る**（howlerは`_sounds`が空だとデコード成功でも`loaderror`を出すため、捨てた音のエラーを画面に出さないよう握り潰す）。恒久的に残るのは**ループ再生**だけ（非ループは`onend`で結局解放される）。ロード完了通知（`join`の待ち解除）は**成否・停止済みを問わず必ず一度出す**（出さないと`[load]`の`Promise.allSettled`が永久に待つ）
