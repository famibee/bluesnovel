...
- [x] **`[set_focus]`のゲームパッド対応を実パッドで確認**（`src/ts/GamepadMng.ts`。ロジックは
  ...
  - [ ] ↑の実機確認中に気付いた別件：`ArrowRight`（`focusMng.next()`）が起点位置によって稀にフォーカスを見失う（`(none)`に落ちる）挙動がある。`Tab`（ブラウザネイティブ）や`ArrowLeft`（`focusMng.prev()`）では再現せず、`fireEvent`/`FocusMng.#move()`という今回の変更が触っていない既存経路の話で、e2e（同じ機構を合成`KeyboardEvent`で駆動）でも再現しない。`playwright-cli`での高速連続キー送信特有のタイミング起因の可能性が高いが未確定。実プレイ速度での再現有無を次回確認
