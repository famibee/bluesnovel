...
- [ ] 【保留】`[jump]`/`[call]`の派生ファイル（`fn+'@'`）差分マージが未移植。本家
      `ScriptIterator.ts:1055-1093`は`fn+'@'`ファイル（例：`chapter1@.sn`）があれば基底ファイルと
      行単位マージする仕組みを持つ（派生側の**空行だけ**を基底側の同一行番号で埋め、行番号を維持。
      `[reload_script]`側にもこの派生ファイルだけキャッシュ破棄する専用処理あり＝本家では意識的に
      維持されている機能）。bluesnovel `ScriptMng.ts`の`#fetchScript()`には無い。
      2026-08-20調査：ドキュメント（tag.html/dev.html）に説明なし、`tmp_esm_uc/doc/prj/`にも
      単一`@`ファイルの実例なし（`@@lang`多言語ファイルは別物・実例あり）。意図的削減の記録は
      見当たらず単純な移植漏れと判断されるが、実運用例が乏しく必要性を確証できないため保留。
      実装するなら中規模（マージ自体は50行未満だが並行ロード書き換え・`[reload_script]`側の
      キャッシュ破棄対応・テストが必要）。着手前に本当に必要か再確認すること
