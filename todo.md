...
- `ArrowRight`/`ArrowLeft`で`[l]`/`[p]`待ちマーカーへフォーカスした状態が、起点位置によって
  `(none)`へ落ちる不具合を修正。前回「稀・タイミング起因」と見送っていたが、`playwright-cli`で
  `tmp_blues`実機に接続し再調査したところ、待ちマーカー自身にフォーカスしている状態からは
  `ArrowRight`/`ArrowLeft`どちらでも100%決定的に再現すると判明。`[event call=true]`の
  `[return]`が`[l]`/`[p]`のタグ位置そのものへ戻る設計のため、`#runStep()`が同じ待ちを
  再処理して`store.wait`がnull→新オブジェクトへ変わり、`TxtLayer.tsx`の待ちマーカー`<span>`が
  作り直されることが原因。作り直された新しい要素へフォーカスを引き継ぐよう
  `TxtLayer.tsx`をローカルに修正（詳細はCHANGELOG.md）
...
- [x] **`[set_focus]`のゲームパッド対応を実パッドで確認**（`src/ts/GamepadMng.ts`。ロジックは
  ...
  - [x] 本文で左右キーでシステムボタンにフォーカスが移った後、本文へ戻れず読み進められなくなる不具合を修正（詳細はCHANGELOG.md 2026-08-11）。`tmp_blues`実機（`playwright-cli`）で`Tab`→新設のフォーカスプロキシ→`Enter`で本文が正しく1行分読み進むことを確認済み
  - [x] ↑の実機確認中に気付いた別件：`ArrowRight`/`ArrowLeft`で`[l]`/`[p]`待ちマーカーへフォーカスした状態が、起点位置によって`(none)`へ落ちる不具合を修正（詳細はCHANGELOG.md 2026-08-11）。「稀・タイミング起因」ではなく、待ちマーカー自身にフォーカスしている状態からの決定的な再現だった。`TxtLayer.tsx`の待ちマーカー`useEffect`にフォーカス追随を追加。`playwright-cli`で`tmp_blues`実機を使い、`ArrowRight`/`ArrowLeft`交互10連打でフォーカスを保持できることを確認済み
  - [ ] （2026-08-11対応：`GamepadMng.ts`にヒステリシス追加、詳細はCHANGELOG.md）ゲームパッドのスティック移動でクリック待ち記号の後ろに一瞬なにかが表示されちらつく不具合の**実パッドでの解消確認**。原因はスティック軸のデッドゾーン境界（`DEAD_ZONE=0.3`）付近の微小な揺れで`ArrowRight`等が連続再送され、フォーカスの輪を余計に進めてしまうことと特定し、ヒステリシス（方向確定後は緩めの`EXIT_ZONE=0.2`で維持判定）で対策した。ロジック検証・シミュレーションでは効果を確認済みだが、`Gamepad API`のエミュレーションがPlaywrightに無いため`#pollAxis()`のポーリングそのものは実パッドでしか確認できない
  - [ ] ゲームパッド・矢印キーによるフォーカス移動後は分かりやすいし仕方ないとしても、マウスクリックでもクリック待ちマークにフォーカスがある事を示す矩形が表示されるようになり、格好悪いので基本は消してほしい
  - [ ] 左ボタンでのフォーカス移動は順番通りだが、右ボタンだと本文フォーカスから先に進まない。キーボードの矢印キーでも再現
    ...
    - ↑の調査で分かったこと：`[l]`/`[p]`待ちマーカーは`ArrowRight`/`ArrowLeft`を押すたび（自分がフォーカス対象か否かに関わらず、まだ同じ`[l]`/`[p]`で止まっている間ずっと）`FocusMng`の輪から`remove()`→`add()`され直しており、`add()`は常に`#aEl`の**末尾**へ積む（`FocusMng.ts:102`）。マーカーが輪の途中にいる構成だと、キーを押すたびに輪の並び順が崩れていく可能性がある。今回のフォーカス消失修正（上記CHANGELOG参照）とは別に、この「並び順が動く」こと自体が本件の原因になっていないか、次回はここから疑う
