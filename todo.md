...
**`ss_000.sn:24`（縦書き）改行位置ズレの本題：原因特定・修正済み**（詳細はCHANGELOG.md
2026-08-18の5件目エントリ参照）。本家`TxtStage.ts:313`の`#SPAN_LAST`（末尾の測定用ダミー要素）が
bluesnovel側に無く、typing完了直後（`[l]`/`[p]`待ちの瞬間）の「末尾の文字にだけ後続が無い」状態で
はみ出し判定が一度もかからない不具合だった。`TxtLayer.tsx`の`mkKinCh()`/`applyKinsoku()`に番兵
エントリ・使い捨てDOM要素を追加して対応。`tsc`/`bun test`（1668件）/`test:e2e`（236 pass、既存の
無関係な4件failのみ）で確認、tmp_blues実機でもcol3が本家と同じ27文字で打ち切られることを確認済み。

**残作業（着手する場合の次回申し送り）**：この修正専用の回帰テストは未追加。既存の
`test/e2e/kinsoku.e2e.ts`は「対が`<br>`で分断されない」不変条件のみを見ており、今回のバグ固有の
条件（typing完了直後、末尾の文字に後続要素が一切無い状態）は捉えていない。tategaki＋bura＋
padding-bottomが絡む境界ケースでのsynthetic fixture作成は追加調査が要るため保留にしてある
（優先度は低——実機確認は済んでいるため）。
