...
void main() {
	...
	// 元絵→モノクロへ progress（0→1）で寄せるランプ型。amp＝最終的な脱色量（0..1、既定 1）。
	//	輝度は Rec.601（本家 pixi greyscale／CSS と同系）。1 タップだけ＝ほぼ無料。keep 既定 true。
	grayscale: `${HEAD}
uniform float amp;
void main() {
	...
	// 元絵→セピアへ progress（0→1）で寄せるランプ型。amp＝最終的な効き（0..1、既定 1）。
	//	行列は CSS sepia(1)／[add_filter] filter=sepia と同一。1 タップだけ＝ほぼ無料。keep 既定 true。
	sepia: `${HEAD}
uniform float amp;
void main() {
	vec4 c = texture2D(uSampler, vTextureCoord);
