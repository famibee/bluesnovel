...
- **`ScriptMng.#refreshCryptoAssets()` が grp レイヤの基本画像・face・`[add_fx tex=]`・
  文字レイヤ `b_pic` を個別に列挙する専用メソッド。**… **見送り**（2026-09-05 調査）。
  理想形は `[load]` 復元後に `chgPic`/`chgBPic`/`addFx` 相当のアクションを合成し、既存の
  `#applyAction()` 経由で流す形（解決ロジックを1箇所に保つ）だが、`aFx` 側は既に解決済みの
  `texSrc` を直接渡しているだけで通常の `addFx`（生の `tex` から解決する経路）とは意味が違い
  アクション種別を新設するかの設計判断が要る。`#applyAction()` は本来 `ScriptEngine.step()` が
  生成した実行時アクションを流す口で、復元用に転用して世代カウンタ等の前提が壊れないかの検証も
  要る＝アクション経路そのものを変える改修になる。得られるのは「新しい fn/src 型フィールドが
  増えるたびの列挙し忘れ防止」のみで、実際に `[add_fx tex=]` が増えた際もレビュー（`/simplify`）
  で発見できており実害は出ていない＝churn > value。
