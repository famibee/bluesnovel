...
- **途中シーンの実機確認は本編ファイルにラベルを足さず、使い捨てテスト用 `.sn` を作る。**
  確認したい `[grp]`〜タグ列を本編からコピーして並べた専用シナリオ
  （例 `doc/prj/script/test_xxx.sn`）を作り、`main.sn` の起動ジャンプだけ一時的にそこへ
  向ける。本編ファイルは無編集のまま（diff ゼロ）で確認でき、確認後はテストファイルを
  消して `main.sn` を戻すだけで済む。ファイル名は `sub.sn` の `[call fn=_*]`
  （`_` 始まりを自動コール）と衝突しない名前にする。
- **本編中盤の `[grp]` へ直接ジャンプすると顔差分未ロードでエラーになりうる。**
  `[add_face]` 群はシナリオ先頭の `[sys_scenario_start]`（`sub.sn` の `[call fn=face*]`
  経由）で初めて登録されるため、それを経ずに `[jump]` すると
  `[add_face] 画像が見つかりません` になる。テストファイル側でジャンプ前に一度
  `[sys_scenario_start]` を呼んでおく。
- **CSS フィルタが効いているかはスクリーンショット目視だけで判断しない。** 白飛び気味の
  背景写真（空・花畑など）は `brightness()` を下げても暗く見えにくく、「効いていない」と
  誤判定しやすい。実際に反映されているかはブラウザ側で
  `document.querySelector('[data-lay=base]').style.filter`（または `getComputedStyle(...)`）
  の値を直接見るのが確実。
