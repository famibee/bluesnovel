...
// 本家 skynovel_esm/src/sn/RubySpliter.ts をそのまま移植したもの
//	（本文のルビ記法`｜…《…》`・`…《…》`・傍点`《*》`を「1文字＋ルビ」の並びへ割る）。
//	相違は T_PutCh を本家 CmnInterface.ts から持って来ずここで定義していることだけ
//	（bluesnovelはpixi前提のCmnInterfaceを持たないため）。
//	test/RubySpliter.test.ts は本家のテストを無改変で持っている
