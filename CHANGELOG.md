...
- [x] **場面転換で演出ありの`[trans]`を連続で打つと一瞬真っ黒がちらつく（対象外レイヤの
  ...
  - 症状：`[grp]`マクロ相当（本家 `doc/prj/script/sub.sn` の `[trans * layer=&dsp_lays_grp]`
    直後に `[trans layer=mes]` を打つ形）で、1本目の演出が正常終了した直後、2本目の演出中に
    背景レイヤが一瞬消えて黒画面になる
  - 原因：`Stage.tsx`のクロスフェード用`useEffect`は「`trans`が`null`に戻ったときだけ」両方の
    ページ板の`opacity`を1へリセットしていた。しかし`[wt]`の直後に別レイヤ対象の`[trans]`を
    連続で打つと、`ScriptMng#runStep()`が同期の`for`ループの中で`finishTrans()`（`trans→null`）
    と続く`startTrans()`（`null→次のtrans`）を続けて呼ぶため、Reactが両方を1回のレンダリングへ
    バッチしてしまい、`trans:null`という中間状態が一度もコミットされない。結果「終了時だけ」
    効くリセットが一度も走らないまま次の演出が始まり、直前の演出でopacity 0まで下がった板が
    そのまま次の演出の下地になって一瞬真っ黒に見えていた（2026-08-11の同系統修正とは別経路）
  - 修正：「終了時だけ」ではなく**演出開始のたびに毎回**両方の板をopacity 1へリセットしてから
    動かす形にした
  - `test/e2e/app/prj_grp/main.sn`の該当timeを一時的に伸ばして実測し、修正前は裏ページの
    opacityがリセットされないまま次の演出に引き継がれること、修正後は常に1へ戻ってから
    始まることを確認した
  - E2E追加（`trans_seq.e2e.ts` + フィクスチャ`prj_transflash`）。演出ありの`[trans]`を`[wt]`
    越しに連続で打ち、2本目の演出開始直後に両方の板のopacityが1近くまで戻っていることを
    確認。修正前のコードでは`back.opacity`が0のまま検出され、確実に落ちることも確認済み

- [x] **凍結issue「ルビ付き行の行間不揃い」を再検証：`margin-block-start`を列の先頭のみへ絞る
  修正（直後のエントリ、`ss_000.sn:24`本題）により、行/列の途中にルビがある場合の不揃いは
  解消したと確認。ただし行/列の先頭にルビが来る場合は依然として直前との間隔が広がる（意図的な
  仕様のため凍結継続）**（2026-08-18）
  - 実データ（`ss_000.sn:24`「安全｜剃刀《かみそり》の刃なんぞが…」列、ルビは列の3文字目＝
    途中）で各表示単位の`getBoundingClientRect().top`を実測。「安」→「全」→「剃刀かみそり」
    →「の」…と続く列内ピッチは28.12〜28.13px（複数文字ルビの親文字ぶんだけ2倍）で完全に均一、
    `marginTop`もすべて`0px`。ルビを挟んでも列内の文字間隔が広がらないことを確認した
  - 一方、`test/e2e/app/prj_ruby`のmargin-block-startテスト用シナリオ（幅固定・monospaceで
    折返しさせ「字《じ》」が行の先頭に来るケース）を実機再現すると、先頭に来たルビには
    `marginTop: 11px`が付き、直前行との間隔だけ広がる（他行のピッチ約31.5pxに対し約48px）。
    これは「`<rt>`が1つ前の行/列に食い込むのを防ぐ」ための必要な余白で、今回の修正でも
    削除できない（削除すると`ss_000.sn:24`本題のバグへ逆戻りする）
  - 結論：凍結issueが指す「ルビ付き行だけ行送りが広がる」不揃いは、**旧実装（先頭かどうかを
    問わず全ルビへ無条件適用）が生んでいた「途中にあるルビでも広がる」ぶんは解消済み**。
    「先頭に来た場合のみ広がる」ぶんは物理的に必要な余白であり解消不可能なので、凍結は
    継続（`todo.md`の凍結issue本文を今回の状況に合わせて更新）

- [x] **`test:e2e`の既知fail5件のうち3件（`waitev.e2e.ts`）を修正：`[p]`待ち中の`[event]`
  call割り込みで`clearOnResume`を凍結する仕様変更（`dfe99ed`）に、テスト用シナリオが
  追随していなかったのが原因と判明**（2026-08-18）
  - `test/e2e/app/prj_wait/main.sn`の`*c2plike`（title.snのクリック待ちオーバーレイ`*c2p`を
    模したラベル）は、`[p]`待ち中に`[event global=true call=true key=x]`で割り込みcallされる
    構成。`dfe99ed`（[l]/[p]待ち中の割り込みで本文が消えないよう`clearOnResume`を凍結する修正）
    以前は「[p]待ち中に別ラベルへcallされると、次の進行で本文がクリアされる」バグ挙動があり、
    テストはこの（バグの）挙動に依存して「くりっくまち」だけが表示されることを期待していた。
    `dfe99ed`でこのバグが直った結果、「むこうりんく」（前のシーンの本文）が消えずに残ったまま
    「くりっくまち」が続けて表示され、テストの期待値と食い違うようになっていた
  - 実際のtitle.snの`*c2p`は別レイヤ（`mes_c2p`）へオーバーレイ表示することで元の本文を
    クリアしない設計だが、テスト用の`*c2plike`は単純化のため同じ`mes`レイヤに直接書いていた。
    `*c2plike`の先頭に`[er]`を追加し、割り込み時に本文をクリアしてから表示する形に修正（本質の
    確認対象である「`[enable_event enabled=false]`中でも`[waitclick]`/Enter/合成KeyboardEventで
    進める」ことには影響しない）
  - 残る1件（`focus.e2e.ts:272`「隠したフレームの中の要素はフォーカスの輪から飛ばされる」）は
    単体worker実行では毎回passし、並列実行時のみ失敗する既存のflaky（過去セッションでも
    同種の記録あり）と確認、対応不要と判断。修正後`bun run test:e2e`は239 pass/1 fail
    （この既知flakyのみ）

- [x] **`ss_000.sn:24`本題：真因は別にあった。ルビ付き文字の`margin-block-start`補正が
  列の先頭以外にも掛かり、列全体が前の列側へ押し出されて隙間になっていた**（2026-08-18、
  さらに続き）
  - 前回（直後の5件目エントリ）の`#SPAN_LAST`番兵修正後、ユーザーから実機で「【ある道具】や
    【の、安全】の間に空行が入る」との指摘。文字数比較（本家とcol1〜col6完全一致を確認済み
    だった）では見えない種類の不具合だった——実際に列のx座標を計測すると、ルビ付き文字を
    含む列の前に、通常42px程度のはずが78px/105pxという大きな隙間があった
  - 原因：ルビ文字が実際に列の先頭に来るかどうかに関わらず、`margin-block-start`補正
    （「ルビが1つ前の列に重なるのを防ぐ」ための処理。`newSpans.forEach`で新規追加spanへ
    無条件に適用していた）が全てのルビ付き文字へ掛かっていた。vertical-rlの
    `margin-block-start`は物理的に「右＝1つ前の列側」を指すため、列の途中にあるルビへの
    marginはその列内の文字間隔を広げるだけ（既知の凍結issue「ルビ付き行の行間不揃い」相当）
    だが、たまたま列の先頭に来た場合は前の列との境界に効いてしまい、列全体を押し出していた
  - 対応：`newSpans.forEach`での先付け処理を削除し、`applyKinsoku()`内で禁則計算
    （`<br>`挿入）が確定した後に、各`<br>`の直後の表示単位（＝実際にその列の先頭に来た文字）
    だけを対象にmargin-block-startを設定し直す方式へ変更（`TxtLayer.tsx`）。禁則計算自体は
    `top`（縦書きの列内位置）のみを見ておりmarginの影響を受けないため、先に禁則を確定させて
    から後付けできる
  - 確認：`bunx tsc`／`bun test`（1668件）green。`bun run test:e2e`は236 pass/5 fail
    （既存の無関係な4件＋今回は`trans.e2e.ts:240`が並列実行時のみのflakyで1件、いずれも
    今回の変更とは無関係と確認）。実機でcol1〜col4の列間隔が78px/105pxから42〜50px
    （正常範囲）に戻ることを実測・スクリーンショットで確認
  - `test/e2e/ruby.e2e.ts`のmargin-block-start検証テストを新仕様に合わせて更新
    （「行/列の先頭でなければmargin無し」に変更）、新規に「行/列の先頭に来たルビにはmarginが
    付く」テストを追加（`prj_ruby/main.sn`に幅固定・monospaceで自然折返しさせるシナリオを追加）
  - 今回の実機比較に使ったplaywright-cli自動化を`.claude/scripts/kinsoku_compare.sh`
    （tmp_blues側）・`kinsoku_compare_esm.sh`（本家tmp_esm_uc側）としてスクリプト化。
    次回からは毎回手探りにせず呼ぶだけで実機比較できる

- [x] **`ss_000.sn:24`本題：原因を特定・修正。本家`#SPAN_LAST`（末尾の測定用ダミー要素）の
  移植漏れと判明**（2026-08-18、同日5件目。前回までの経緯は直後の4エントリ参照）
  - 直前の4件目エントリの「根本原因は未特定」を受けて再開。事前仮説（複数文字ルビがkc配列で
    N+1要素になり`mkKinCh()`/`i2pi()`のインデックス計算がズレる）を`kc`/`idx`/`sub`/`xy`配列を
    実機でダンプして検証したが、**この仮説は誤りだった**——ルビ展開・インデックス対応・
    `Hyphenation.ts`のスキャンロジックはいずれも設計通り正しく動作していた
  - 真因：本家`skynovel_esm/src/sn/TxtStage.ts:313`は表示中テキストの末尾へ毎回
    `#SPAN_LAST = '<span class="sn_ch sn_ch_last">&emsp;</span>'`という実DOM要素を足してから
    禁則計測している（元コメントは「末尾改行削除挙動対策」だが、副次的に「表示中の最後の1文字」
    にも“次に何か置いたら列からはみ出すか”を判定する後読み材料を与える役目を果たしている）。
    bluesnovelにはこの仕組みが無く、複数文字ルビ「安全｜剃刀《かみそり》」を含む列（禁則の
    巻き戻しがそれ以前に一度も要らなかった列）で、末尾の「──」が一度もはみ出し判定にかからない
    まま詰め込まれていた
  - 対応：`src/components/TxtLayer.tsx`の`mkKinCh()`末尾に番兵エントリ（`idx=-1`で識別）を1つ追加、
    `applyKinsoku()`側で使い捨ての`<span style="display:inline-block"> </span>`をこの番兵の実測に
    使い、`finally`で必ず除去するようにした（本家`#SPAN_LAST`相当。文字送り演出・`cache`/
    `childNodes`数の前提には一切関与させない）
  - 確認：`bunx tsc --noEmit --incremental false`／`bun test`（1668件）とも green。
    `bun run test:e2e`は236 pass/4 fail だが、この4件は修正前（`git stash`でこのセッションの
    差分を除いたHEAD）でも同じ理由で失敗する既存の不具合／flaky（`waitev.e2e.ts`3件は
    `[enable_event]`関連の既存回帰、`trans.e2e.ts:267`は並列実行時のみのflakyなピクセル比較）で
    あることを確認済み、今回の修正とは無関係。tmp_blues実機（headless、todo.md記載の再現手順）で
    再検証し、col3が「か」（27文字目）で正しく打ち切られ「──」が新しい列（col4）へ押し出される
    ことを確認した（修正前は29文字分詰め込んでいた）
  - 未対応：この修正のための専用回帰テストは追加していない（`kinsoku.e2e.ts`の既存テストは
    「対が`<br>`で分断されない」不変条件のみで、「typing途中で末尾の文字にだけ後続が無い」という
    このバグ固有の条件は捉えていない。tategaki＋bura＋padding-bottomの相互作用が絡む境界ケースで
    synthetic fixtureでの確実な再現条件を作るのに追加調査が要るため、実機再現での確認に留めた）
  - `todo.md`引き継ぎの本題項目を削除（解決済み）

- [x] **`ss_000.sn:24`本題の続き：決定的に再現するheadless自動化ハーネスを構築、分岐点を1文字単位まで
  特定。ただし根本原因は依然未解決**（2026-08-18、同日4件目）
  - Playwrightの`playwright`パッケージを直接使い、tmp_blues（:5173）を新規コンテキストで開き
    クリック内容一致（`/来るのか─+\s*$/`、クリック回数固定ではなく）でチェックポイントへ到達する
    自動化スクリプトを作成。**headless Chromiumでは10/10試行すべてが同一のバグ結果**（col3が
    `か`で終わらず`──`まで29字ぶん収める）で、**再現性が揺らぐ「flaky」ではなく決定的**と判明。
    過去セッションで「ブレる」と観測されていたのは、headed/headless混在や実行環境差によるものだった
    可能性が高い（今回はheadless単体では常に同じ結果になった）
  - 本家（tmp_esm_uc、`npx vite --port 5174`で起動）はheadless Chromiumだとcanvasが常に真っ黒
    （pixiのWebGL出力がheadlessで無効化される既知の制限、`.sn_ch`要素数0で確認）。`headless: false`
    （headed）で正常に描画されることを確認、以後本家の計測はheaded必須
  - 本家はタイトル画面までpixiで描画するためDOM上のテキストが無く、`document.body.innerText`等では
    操作できない。座標クリック（再生ボタン相当の▷を(480,353)、「最初から」相当を(275,390)）で
    タイトル→本編へ進める必要がある。実際の文字レイアウトは本家自身の禁則計測用の隠しDOM
    （`.sn_tx`/`.sn_ch`、`TxtLayer.ts`のhyph()が使うのと同じ要素）から`getBoundingClientRect()`で
    読み取れる（画面には見えないが実座標を持つ）
  - 両エンジンとも同じチェックポイントで`.sn_ch`／bluesnovelの表示単位spanの**1文字ごとのx,y座標を
    比較**した結果、col3の「安」から「か」（27文字目）まで**全文字が1px単位で完全一致**（同じ
    x=183〜186相当、同じy刻み28→613）。分岐点は「か」の直後の1点のみ：本家はここで新しい列へ
    折り返す（列の使用量が636pxで打ち切り）が、bluesnovelは折り返さず`─`をy=636、もう1つの`─`を
    y=658に同じ列内へ続けて配置する
  - コンテナのCSS計算値（`getComputedStyle()`）を比較：箱の矩形・`font-size`・`line-height`・
    `writing-mode`・`box-sizing`は完全一致。唯一の差は`padding-bottom`（本家22px、bluesnovel24px。
    どちらも`bura`モードの「行末2文字ぶんぶら下げ領域確保」計算 `pb = 30 - font_size*2 = -18px`
    という無効値がCSSにドロップされた際のフォールバック値で、フォールバック元がエンジンごとに
    異なるため2px差になっている）
  - **この2px差を`padding-bottom: 22px !important`で本家に合わせて強制しても、折り返し結果は
    1pxも変わらなかった**（同じ2文字が同じ列にはみ出す）。2px差はバグの原因ではないと実測で否定
  - `padding-bottom`の効き方自体をより広い範囲で確認：24px（既定）〜40pxまでは列境界に一切影響
    なし、46pxで急に27文字目より手前（26文字）まで後退——27・28文字目の状態を飛び越える**非単調な
    挙動**だった。`bura`ぶら下げ計算自体が`padding-bottom`を参照する構造のため、この値を人為的に
    動かす実験はアルゴリズム自体を乱しており今後は避けること
  - **根本原因は未特定のまま**：1文字ぶん（約23px）の「使える高さ」の差がどこから来るかは、単純な
    箱の高さ計算（`height - padding`）では説明がつかない値になっており（両エンジンとも理論値を
    超えて描画されている）、禁則（`Hyphenation.ts`のぶら下げロジック）とCSSレイアウトの相互作用まで
    含めて追う必要がある。次回は`todo.md`引き継ぎの「本家マークアップを実機`charsRef`へ直接
    流し込む」実験（今回は計測比較のみで未着手）から再開する
  - 副産物：上記のheadless自動化ハーケスはスクラッチパッドのみに置き、セッション終了時に削除した
    （リポジトリには残していない）。再現手順（URL・セレクタ・チェックポイント正規表現・本家の
    座標クリック手順）は`todo.md`引き継ぎに残す
  - `TxtLayer.tsx`への実験的な変更（`position: relative`・`padding-bottom`上書き）はすべて検証後に
    元へ戻し、diffはこのセッション開始時点と同じ状態

- [x] **`ss_000.sn:24`本題の続き：ユーザー提案の「本家へ完全移植→段階的に戻して原因箇所を特定」
  方針で「inline/inline-block」仮説を検証・否定**（2026-08-18、同日3件目）
  - 本家`TxtLayer.ts:#tagCh_sub()`（複数文字ルビの枝、739-751行）を読み直したところ、内側の
    親文字spanには`class="sn_ch"`（CSSで`display:inline-block`）を当てつつ、**同じ要素の
    インラインstyleで`display: inline;`を明示して上書きしている**と判明（コメント「[span]区間外の
    ルビが浮く」対策）。つまりbluesnovelの元コード（内側spanにdisplay無指定＝UA既定の`inline`）は
    元々本家と一致しており、直前セッションで加えた「内側spanへの`inline-block`明示」（今の
    差分には残っていない・実機検証後に撤回済み）は逆方向の誤修正だった
  - 検証のため、本家風マークアップとbluesnovel風マークアップ（複数文字ルビ「剃刀《かみそり》」を
    含む同一文字列）を同一ページに並べて`getBoundingClientRect()`を比較する分離テストを実施。
    **両者の文字ごとの座標は1px単位で完全に同一**となり、inline/inline-block差はcol3のブレの
    原因ではないと確定
  - ただしこの分離テストは簡略化したCSS（`display:flex; flex-wrap:wrap`を手で再現）では実機の
    自然折返し自体が発生しなかった（全文字が1列にオーバーフローするだけで新しい列へ折り返さ
    なかった）。実機の`charsRef`要素が持つ正確な`flex-direction`等を再現できておらず、
    「本家へ完全移植して確認」を正しくやるには**簡略化した分離divではなく実機のコンテナへ直接
    本家風マークアップを流し込む**必要がある（次回への申し送り）
  - 確定している事実（再掲）：col1・col2（複数文字ルビ無し）は毎回本家と一致。col3（複数文字ルビ
    「剃刀《かみそり》」を含む）でだけbluesnovel側の結果がブレる。本家は毎回一致してcol3を
    「か」で切る
  - `todo.md`引き継ぎを更新

- [x] **【訂正】直前のエントリ「バグは存在しなかった」は誤り。実際は再現する（本家一致せず）と
  判明、`ss_000.sn:24`本題は未解決のまま**（2026-08-18）
  - 直前のエントリを書いた直後、ユーザーが実際のブラウザ操作で「col3に`──`が含まれる」誤った
    画面（=まさに元の不具合そのもの）のスクリーンショットを提示。「解消していない」との指摘を
    受けて再検証したところ、**`playwright-cli`で同じチェックポイントを踏み直すたびに結果が
    ぶれる**ことが判明（ある回は本家と完全一致、別の回は`col3`に`──`まで29字ぶん収まる）。
    直前のエントリの「完全一致」は複数回のうち偶然一致した1回だけを見て断定したもので、
    再現性の検証が不足していた
  - フォント読み込みレース（`Font.ts addFontFaces()`が`@font-face`の読み込み完了を待たない）を
    疑い、`document.fonts.ready`後に禁則を掛け直すuseEffectを追加して検証したが、
    **`document.fonts.check()`がtrueな状態（フォント読み込み確実に完了）でも不具合が再現する**
    ことを確認したため、この仮説は誤りと判明・当該useEffectは撤回（追加しても症状が直らない
    ことを実機確認した上で削除）
  - 本家を`playwright-cli`で同じチェックポイントまで正確に進めて比較したところ、**本家は毎回
    必ずcol3を「か」で切り、`──`は次列（col4）から**であり、bluesnovel側だけが結果がぶれる
    （本家に発生しないbluesnovel固有のブレ）。col1・col2（複数文字ルビ無し）は毎回両者一致する
    一方、col3（複数文字ルビ「剃刀《かみそり》」を含む）でだけbluesnovel側がブレることから、
    `elCh()`が複数文字ルビ親文字を包む外側`<span style="display:inline-block">`が「2文字ぶんの
    高さを持つ1個の原子的な箱」になっている点（本家は親文字を1文字ずつ独立した`inline-block`に
    する）が、列の残り高さぎりぎりで実施される自然折返し判定をサブピクセル単位で不安定にして
    いる可能性が高い（実測でこの列の使用量は利用可能高さの714px logicalに対し726px logical相当
    と、余裕がほぼ無い境界ケース）。`Stage.tsx`の`transform: scale(cvsScale)`（非整数）を介した
    測定という点も、丸め誤差の発生源として無視できない
  - 未特定：上記は仮説であり確証は無い。特定するには「複数文字ルビ親文字を含む列が、利用可能
    高さぎりぎりに達するケース」に絞った再現条件の切り分けと、`cvsScale`を変えた時に結果が
    変わるかの検証が必要（今回のセッション工数を超えるため次回に持ち越し）
  - `todo.md`の本題項目を「解決済み」から「未解決・再現性あり」へ戻す

- [x] **【誤り。上記訂正エントリ参照】`ss_000.sn:24`（縦書き）改行位置ズレの本題：3セッションに
  渡り「col3のみ本家とズレる」としていた観測そのものが誤りと判明、実際は完全一致。バグは
  存在しなかった**（2026-08-18）
  - 前回セッションまでの結論「col1・col2は本家と一致（27文字・26文字）、col3のみズレる（──を
    含め29行 vs 本家27行）」は、**2エンジンを別々に手動クリックで同じ箇所まで進めた際、クリック
    回数が揃っておらず実際には台本上の別の位置同士を比較していた**ことによる測定アーティファクト
    と判明。原因は`main.sn:26-27`の`[char2macro char=@ name=lr]`（`lr`マクロ＝`[l]`相当の行末待ち）
    ＋`[char2macro char=\ name=plc]`（`\`＝場面転換）——本文中に地の文として現れる`@`と`\`が
    禁則とは無関係にクリック待ちを挟む記号だと認識しないまま両エンジンを進めていたため、
    「本家は2個目の`──@`で止まる」「bluesnovelは3個目まで進んでいた」という状態を「同じ箇所」
    として比較し、ズレて見えていた
  - `playwright-cli`で本家(:5174)・bluesnovel(:5173)を`localStorage`クリア済みの新規セッションから、
    DOM文字列の内容一致（`のだが──`で終わる、等）を条件に1クリックずつ確実に停止点を合わせて
    再計測。`ss_000.sn:24`の段落全文（`どうして俺が…`〜`…ちがひない。`、134文字・5列ぶん）の
    列ごとの文字送り座標（`getBoundingClientRect()`）を全数突き合わせた結果、col1〜col5の
    文字数・改行位置が**1文字残らず完全一致**（27・26・27・27・27文字）。6列目の末尾のみ12文字
    対13文字の差があったが、場面転換直前の非表示領域の計測誤差と見られ、実文字列内容
    （`それもこれもやつぱり同じやうなことにちがひない。`）自体は同一
  - 2026-08-17に修正した複数文字ルビ親文字の禁則判定漏れ（`TxtLayer.tsx mkKinCh()`）は独立した
    実バグとして正しく直っており、本題とは無関係（そのまま維持）
  - 教訓：**本文中の記号（`@`/`\`）が`[char2macro]`でクリック待ち・場面転換にマッピングされている
    台本を2エンジンで手動比較する際は、クリック回数でなくDOM文字列の内容一致で停止点を揃える
    こと**。todo.mdの「要検証：縦書きグリフ描画不具合」もこの一連の調査中に発生したもので、
    今回の件と合わせて調査手法自体の見直しが必要だった
  - `bun test`（1668件）・`bunx tsc --noEmit --incremental false`とも回帰なし
  - `todo.md`引き継ぎの本題項目を削除（解決済み）
...
- [x] **`ss_000.sn:24`の改行位置が本家とズレる件を実機比較で調査**（2026-08-12）
  - `tmp_blues`（vite :5173）と`tmp_esm_uc`（本家、vite :5174）を`playwright-cli`で並行起動し、
    同じシナリオ位置（「安全｜剃刀《かみそり》の刃なんぞが、千里眼のやうに思ひ浮んで来るのか──」）
    まで進めて実機比較。最初の2列（「どうして俺が…」「道具のうちの…」）は完全一致し、3列目で
    本家は「…来るのか」で折り返して「──」を次の列の先頭へ送るのに対し、bluesnovelは同じ列に
    「──」まで収めてしまう、ちょうど2文字ぶんのズレだった
  - ルビ（｜剃刀《かみそり》）自体の折返し位置は両エンジンで一致しており、ズレの原因はルビ文字
    分割の違い（`T_CH`/`elCh()`が親文字を1ブロックのまま扱うこと）ではなく、1列に収まる文字数
    そのものの差（ブラウザのCSS自動折返し vs 本家のRange計測、および`box-sizing`解釈差）と判断。
    これは`[lay pl=/pr=/pt=/pb=]`実装時（2026-08-10）に既に「厳密な行数一致はブラウザの自動折返しと
    本家のRange計測ベース折返しの差もあり達成が難しく、実用上の近似止まりでよい」と結論済みの制約と
    同じ系統
...
- [x] **`ss_000.sn:24`の禁則を実データ回帰テスト化、あわせて縦書きで自動改行直後のグリフが消える描画不具合を発見・調査**（2026-08-17）
  - `tmp_esm_uc`側で`prj.json`の`"dumpHtm": true`（本家`TxtStage.ts`の`debug.dumpHtm`）を有効化して
    実機出力したhtmlを受け取り、`ss_000.sn:24`の禁則結果（分割禁止文字「─」の対「──」を割らずに
    次の列頭へ送る）を正典として利用した
  - `test/e2e/app/prj_kinsoku/main.sn`・`test/e2e/kinsoku.e2e.ts`に実データ回帰テストを追加：実際の
    一節「来るのか──お前は」をwidth:5emで流し込み、「─」の対の間に`<br>`が挟まらないこと（分割禁止
    の不変条件）を検証。折り返し**位置**そのもの（何文字目で折り返すか）はmonospaceフォントの実測px
    次第で環境依存になるため固定しない（`bun test`・`bunx tsc --noEmit`・`bun run test:e2e -- kinsoku.e2e.ts`
    全て確認済み）
  - あわせて`tmp_blues`（bluesnovel）実機を`playwright-cli`で`tmp_esm_uc`と比較したところ、
    折り返し**位置**の差（2026-08-12エントリで確認済みの既知制約。ブラウザの自動折返し vs 本家の
    Range計測ベース折返しの差）とは別に、**分割禁止で次の列頭へ送られた対の2文字目のグリフが
    描画されない**現象を新規発見した。DOM上は座標・`opacity`・`font-*`系computedStyleすべて前後の
    正常な文字と一致しており、`textContent`書き換え・`display`トグル・`opacity`トグル・`transform`
    除去のいずれを試しても再現後は直らない（＝取りこぼしたペイントの単純な再フラッシュでは直らない）。
    最小HTML再現はシステムフォントでは不成立だったため、プロジェクト同梱フォント`ipamjm`固有の
    縦書きグリフ差し替え（`vert`/`vrt2`）絡みを疑っているが未確定。詳細な再現手順・切り分け結果は
    `todo.md`「挙動の詰め・実機確認」に記載して未解決のまま持ち越した
  - bluesnovel側の`dumpHtm`相当機能（確定後DOMをファイル出力するデバッグ機能）は、`outputFile` IPC
    自体は既存（`[log]`等で利用）なものの、`debug`設定フラグの枠組みも`const.sn.isApp`の実値切替も
    未整備で`TxtLayer.tsx`（React）側へのフック追加も要り、本家との表示比較は都度`playwright-cli`の
    実機比較で足りているため、専用機能としての実装は工数に見合わず見送った（ユーザー確認済み）
  - 調査用に置いてもらった`dumpHtm/`（本家の実機出力htm一式、`.gitignore`対象外の作業ディレクトリ）
    は役目を終えたため削除した
  - `todo.md`には解決した項目は無い（発見した描画不具合を新規追加した）

- [x] **縦書き禁則グリフ描画不具合の追試：3回の手動再現ではいずれも非再現、間欠的な現象と判明**（2026-08-17）
  - 前エントリで発見した「分割禁止で次の列頭へ送られた対の2文字目のグリフが描画されない」現象について、
    `playwright-cli`で`tmp_blues`実機を対象に、キャッシュの効かない新規ブラウザプロセス（Chrome）を
    都度起動し、タイトル→「最初から」→手動ペースのクリック進行で`ss_000.sn:24`の同じ箇所
    （「…来るのか──お前は…」の禁則ペア）まで3回独立に到達し、都度対象spanのDOM座標を取得して
    スクリーンショットの該当ピクセルを拡大確認したが、3回とも対の2文字が連続した1本線として正しく
    描画され、再現しなかった（`run-code`での高速クリック連打は逆にCSS未適用のFOUC状態を誘発し
    比較対象として不適と判明したため、対話的な`playwright-cli`コマンドでの手動ペース操作に統一）
  - WebKitでの追試も試みたが、このサンドボックス環境にはWebKitのブラウザバイナリが未インストールで
    断念（Chromeのみでの検証）
  - 前回の`vert`/`vrt2`仮説はこの非再現により検証段階まで進めなかった。発生条件が非常に限定的な
    間欠的レース（初回ペイントのタイミング依存等）である可能性が高いと判断
  - ユーザー判断：パッチ的な再現追跡はこれ以上行わず、縦書き文字レイヤの描画方式（1文字ずつを
    `inline-block`の`span`に分解し、ブラウザの`writing-mode`／OpenTypeの`vert`グリフ差し替えに
    任せる現行方式）自体を仕様を満たす形で全面的に作り替える方策を、別セッションで検討する。
    今回はその方針を`todo.md`へ記録するに留めた

- [x] **縦書きグリフ描画不具合の原因調査：本家との差分を洗い出し、文字出現演出をGSAPからWeb Animations APIへ置き換え**（2026-08-17）
  - 前々エントリ・前エントリで持ち越した「間欠的な描画欠落」について、本家`skynovel_esm`と
    bluesnovelの実装をサブエージェントで並行調査し比較。**本家も文字本体はDOM描画**で、禁則も
    同じ「計測→`<br>`挿入→再計測」ループと判明（pixiは背景・文中画像・リンク判定にのみ使う）。
    方式自体は同じなので、bluesnovel固有の差分だけが容疑者になるという原則で切り分けた
  - 差分のうち最有力と判定したのは**GSAPの既定`force3D:"auto"`**：アニメ進度が0でも1でもない間
    だけ`transform: matrix3d(...)`を書き、Chromiumのレイヤ昇格/降格をアニメのたびに引き起こす。
    本家のkeyframesは2D変形のみでこれが無い。「一度壊れると`textContent`書き換えでも直らない」
    という既存の切り分け結果とも、壊れているのがラスタタイルでなくshaped-run/glyphキャッシュ側
    だとすれば整合する
  - ここで立ち止まり、「GSAPを使う技術的必然性があるか」をユーザーと検証。本家のスキップ実装
    （`TxtStage.ts:590-597 skipChIn()`）は文字spanのクラスを一括差し替えるだけで完結しており、
    JSトゥイーンエンジンを要していないと判明。bluesnovelがGSAPを使っていた理由は「文字送りを
    既にGSAPで回している」というアーキテクチャ上の選好のみで、必然性は無かった
  - 代替案を3つ比較（本家のCSS keyframes動的注入をそのまま移植／静的1個の`@keyframes`をCSS変数で
    パラメータ化／ブラウザ標準のWeb Animations API）した結果、**Web Animations API
    （`Element.animate()`）**を採用。理由：GSAPの3D昇格問題が構造的に存在しない（ブラウザの
    コンポジタが補間を担当）、追加ライブラリ不要、現状の`TxtLayer.tsx`のコード形（1文字ごとに
    トゥイーンを積む・完了コールバック・キル）をほぼ保ったまま置き換えられる。`[tsy]`/`[trans]`/
    `[quake]`/音声フェードはGSAPのまま
  - `[ch_in_style]`の`to`が常にCSSの初期値（`opacity:1, transform:none`）と一致する設計を活かし、
    `options.fill:'backwards'`と組み合わせることで、旧`CH_END`定数＋`clearProps`という明示的な
    「終端確定」処理が丸ごと不要になった（`.cancel()`すれば自動的に素のDOM既定値へ戻る）。
    easingもCSSの`<easing-function>`構文をそのまま渡せるようになり、GSAP用のease名変換テーブル
    （`cubic-bezier()`/`steps()`を諦めて既定へ倒していた）が不要に。ただし`el.animate()`は無効な
    easing文字列に同期的にTypeErrorを投げるため、`ChStyle.ts`の`chStyleEase()`を正規表現による
    妥当性検査＋フォールバックへ作り直した
  - 完了検知に新しい落とし穴：GSAPの`.kill()`は`onComplete`を発火しないが、Web Animations APIの
    `Animation.finished`は`.cancel()`後も`Promise.allSettled()`ではresolveしてしまうため、古い
    バッチの完了ハンドラが新しいバッチの`isTyping`を誤って下ろす競合がありうる。世代カウンタ
    （`genRef`）で自分の世代がまだ最新かを確認してから`setIsTyping(false)`する対策を入れた
  - 副次的に見つけた実バグ2件も修正：①`[lay]`のデザインモード用の下地`sty4Moveable`
    （`Stage.tsx`）が、デザインモード無効中（`ENA_DESIGN_MODE=false`の現状）にもかかわらず全
    レイヤへ恒等`transform`を書き続けていたため、`isDesignMode`時のみに限定。②ルビの
    `marginBlockStart`計測（`TxtLayer.tsx`）が`getBoundingClientRect().height`（祖先の
    `transform: scale(cvsScale)`を含む）を使っており、`cvsScale!==1`のウインドウで実際にズレる
    ことをE2Eの失敗で確認・`offsetHeight`（transform非依存）へ修正した（`ruby.e2e.ts`の該当
    テストも比較基準を合わせて修正）
  - **①の副作用で新しい回帰が発生**：ユーザーが`tmp_blues`実機（2ページ目「これは信じて〜」）で
    文字レイヤの背景（`[lay b_pic=…]`の枠画像）が表示されなくなっていると報告。原因は
    `TxtLayer.tsx`の`styTxt`が`&::before`（背景画像疑似要素）に`z-index: -1`を当てているが、
    それを自身の子として背面に留めるスタッキングコンテキストを明示的に持っていなかったこと。
    従来は`sty4Moveable`が全レイヤへ常時書いていた恒等`transform`が**偶然**スタッキング
    コンテキストを作っていたため気付かれていなかった副作用で、①でそれを無くしたことにより
    `::before`が要素の外（立ち絵レイヤの背後）へ回り込んでしまっていた。`transform`の副作用に
    頼らず、目的（背面固定）に合った`isolation: isolate`を`styTxt`へ明示的に追加して解消。
    `playwright-cli`で`tmp_blues`実機を再確認し、変更前（`git stash`で退避して比較）と同じ見た目に
    戻ったことを確認した
  - `test/e2e/chstyle.e2e.ts`の`freeze()`/`advance()`をWeb Animations API向けに書き換え。GSAPの
    「グローバルなアニメ時計を先に止めておける」性質に相当するAPIが無い（`document.getAnimations()`
    は呼び出し時点で存在するAnimationしか返さない）ため、凍結は**キー押下の直後**に行う形へ変更
    （正確性はGSAP版よりわずかに劣るが、既存アサーションの余裕（数十ms単位）で実害は無い見込み）
  - 単体テスト全件・E2E全件（既存の無関係なフレーク3件を除く）で回帰なしを確認
  - `tmp_blues`実機を`playwright-cli`で確認：`ss_000.sn:24`の該当箇所（「…千里眼のやうに思ひ浮んで
    来るのか」の次の列「──お前はそれが…」）まで読み進め、送られた「─」の対の両方のspanで
    `getComputedStyle().transform`が厳密に`'none'`（GSAP時代の`matrix3d(...)`残留なし）であることと、
    クロップ画像でペアが継ぎ目なく1本の線として描画されていることを確認した。**間欠的な不具合の
    性質上、この1回の非再現をもって解消の確証とはできない**（過去にも複数回の非再現後に発生した
    実績があるため）が、少なくとも容疑としていたGSAPの3D昇格由来のtransformは構造的に発生し得なく
    なったことをコードレベル・実機レベルの両方で確認できた。設計の要点は`.claude/docs/PITFALLS.md`
    へ記録した
  - `todo.md`「挙動の詰め・実機確認」の該当項目（縦書きグリフ描画不具合）を消化

- [x] **縦書きグリフ描画不具合：「フォント読み込みレース」仮説を検証、ネットワーク遅延では非再現。実機自動周回は未達で持ち越し**（2026-08-17）
  - 前回セッションの引き継ぎ（`willChange`ナッジの効果確認）は、バグが間欠的（過去に3回中3回非再現の
    実績あり）なため手動再現の再確認では統計的に判断がつかないと判断し、着手しなかった
  - 新しい仮説を発見：`src/`全体を`grep`した結果、**`document.fonts`関連のコードが一切存在しない**
    （`src/ts/Font.ts`の`addFontFaces()`は`@font-face`の`<style>`注入のみで、フォント読み込み完了を
    待つ処理が無い）。`ipamjm.ttf`は46MBと巨大なため、コールドキャッシュ時に禁則の折り返し計測
    （測る→`<br>`挿入→再計測ループ）がフォールバックフォントで行われた後にフォントスワップが
    起きる「レース」が起きうるのではという仮説
  - 検証用フィクスチャ`test/e2e/app/prj_vertglyph/`を新設（`ipamjm.ttf`は46MBあり`TESTING.md`の
    「バイナリは極力置かない」方針に反するため**未コミット・`.gitignore`対象**。tmp_blues実テンプレの
    `txt_lay_v_left`マクロと同じCSS・同じ禁則ペア文字を使い、実フォントでの再現条件を最小化）
  - Playwrightの`page.route()`でフォントファイル応答にのみ人為的な遅延（0/500/1500/3000/6000ms）を
    挿入し、17トライアル実施したが**一度も再現せず**（対象'─'span全てのink比率が全トライアルで
    完全同一値）。ただし実際の発生環境は`localhost`のvite devサーバーであり、実験のようなネットワーク
    遅延そのものは発生環境の実情と乖離している可能性がある。仮説自体を否定する強い証拠とまでは
    言えない
  - 実アプリ（`tmp_blues`）を直接自動周回する方針へ切替えたが、**タイトル画面からの自動遷移で行き
    詰まり未達**：ヘッドレスPlaywrightで「最初から」ボタンをクリックしても反応せず、
    `document.elementFromPoint()`で調べたところ、空の文字レイヤ（`data-lay="mes_c2p"`、
    `theme/_c2p.png`）がステージ全面（1024×768）を覆いクリックを奪っていることが判明。
    ヘッドレス特有の座標クリックの問題か、tmp_blues側の軽微な実バグ（本題ではないため深追いせず）
    かは未切り分け
  - 副産物として、Playwrightスクリーンショット（PNG）を外部ライブラリ無しで読み、矩形領域の
    「背景色との差分がある画素」比率を計算するユーティリティ（最小PNGデコーダ＋ink比率判定）を
    作成した。**セッションのスクラッチパッドにのみ存在し、リポジトリには保存していない**
    （設計：`zlib.inflateSync`でIDAT展開→フィルタ複合（type0〜4対応）→4隅平均を背景色とみなし
    ユークリッド距離がしきい値超の画素を「インク」とカウント。1色文字spanの矩形クロップに対して
    実測`0.0303`で全トライアル一致するなど十分な感度を確認済み）。次回同種のツールが要る場合は
    このアプローチ（≒80行程度）を再実装すればよい
  - `todo.md`引き継ぎを更新（このエントリの内容に差し替え）

- [x] **縦書きグリフ描画不具合：タイトル→本文の自動遷移を実現し実アプリ自動周回を実施、80試行で非再現**（2026-08-17）
  - 前回の持ち越し課題「ヘッドレスPlaywrightで『最初から』ボタンが反応しない（`mes_c2p`レイヤが
    クリックを奪う）」をサブエージェントで調査した結果、**バグでなく仕様どおりの2段階クリックが
    必要なだけ**と判明。`theme/title.sn:22-46`の`*c2p`は`const.sn.needClick2Play`
    （`SndMng.ts:59`、`AudioContext.state==='suspended'`を見るだけ）が真のとき全面オーバーレイ
    （`mes_c2p`）を出し`[waitclick]`で1回目のクリックを消費してから`enable_event`を戻す設計。
    Playwrightのクリックはheadless/headfulを問わずChromiumに信頼済みユーザー操作として渡るため
    headless特有の差異ではない。「1回目クリックでオーバーレイ消去→2回目クリックで実際のボタン」の
    2段階操作へ自動化スクリプトを直したところ、タイトル→本文への自動遷移が安定して通るようになった
  - 副産物として、前回セッションの自作PNGデコーダ（zlibの`inflateSync`＋フィルタ復号、
    スクラッチパッドのみに存在し失われていたもの）を再実装する過程で、**unfilter実装のバグ
    （常にink比率0を返す）を発見**。sipsコマンド（macOS標準）でPNG→BMP変換してから走査する方式に
    設計変更し、既知の非ゼロ値で動作確認した上で採用した（自作PNGデコーダより単純で確実）
  - `ss_000.sn:24`（既知の禁則ペア発見箇所、複数ページ読み進めが必要）でなく、**冒頭7行目**
    「桜の樹の下に──は屍体が埋まつてゐる！」に禁則ペア「──」が最初から含まれることを利用し、
    タイトル→本文自動遷移直後にこのペアのink比率を測る形にして周回コストを削減した
  - 新規ブラウザコンテキスト（コールドキャッシュ相当）で**80試行**（30＋50、実行はスクラッチパッドの
    Node.jsスクリプト。恒久e2eテストとしては残していない）実施したが**1回も再現しなかった**。
    手動3回・ネットワーク遅延注入17回（前回セッション）と合わせ、累計100試行超が非再現
  - 自動化基盤（2段階クリックでのタイトル→本文遷移、sips経由のink比率測定）は次回再利用できる
    設計だが、コードは今回もスクラッチパッドに置いたのみでリポジトリには残していない
    （前回と同じ轍。再実装コストは今回小さかったため許容したが、次回また要る場合は
    `test/e2e/app/prj_vertglyph/`と組にして恒久化を検討してよい）
  - ユーザー判断：単純な周回では発生条件に届いていない可能性が高く、これ以上同条件での回数積み
    増しは費用対効果が悪いため、ここで一旦区切り。`todo.md`へ次の方向性（CPU負荷を掛けた再試行／
    `ss_000.sn:24`・長時間読み進めでの再試行）の選択肢を記録するに留めた

- [x] **`ss_000.sn:24`改行位置ズレの本題調査へ復帰、複数文字ルビの禁則判定漏れを発見・修正**（2026-08-17）
  - 前回セッション（同日3セッション目）でClaude自己発見バグ（縦書きグリフ描画不具合）の追跡に
    3セッション費やし本題が放置されていた件を`todo.md`へ出自つきで記録し直し、本題
    （改行位置ズレの本家一致）へ復帰
  - **前回セッションの仮説（`marginBlockStart`補正がルビ行の行送り幅を2倍に膨張させている）は
    誤りと判明・撤回**：実機で`marginBlockStart`のcomputed値を確認したところ、これは物理的に
    `margin-right`（縦書きでは幅＝列方向）に対応し、文字の縦方向の積み上げ（top座標の刻み）には
    影響していなかった。前回「2倍に膨張」と見えたのは、複数文字にまたがるルビ親文字
    （例：「剃刀」）が本家では文字ごとに個別DOM要素、bluesnovelでは1つの表示単位（1要素）に
    まとまっているという構造差を、比較粒度を揃えずに見ていたための誤認だった（消費する行数
    自体は両者とも2行で同じ）
  - `padding-bottom: -18px`（`pb = 30 - font_size*2`、ぶら下げ用の負paddingだが本家・bluesnovel
    双方でCSS的に無効・無視される）の実効フォールバック値は本家22px・bluesnovel24pxで実測差2px
    （可用列高さ716px対714px）と判明。前回セッションの「無視できる差」判断が正しかったことを確認
  - **実バグを1件発見・修正**：`TxtLayer.tsx`の`mkKinCh()`が、複数文字にまたがるルビ親文字
    （例：安全｜剃刀《かみそり》の「剃刀」）について`ch.c.at(0)`で先頭1文字しか禁則判定配列に
    積んでおらず、2文字目以降が丸ごと禁則スキャンから欠落していた。本家は`#tagCh_sub()`で
    親文字を1文字ずつ個別`<span>`に分けるため各文字が独立した判定単位になるが、bluesnovelは
    1つのテキストノードにまとめていたため個別測定ができなかった
  - 対応：`elCh()`で複数文字ルビの親文字（`r !== undefined && chars.length > 1 && ! tcy`の場合
    のみ）を1文字ずつ内側`<span>`へ分割（本家`#tagCh_sub()`と同じ発想。**文字送り演出は変更しない**
    ——演出対象は従来どおり表示単位＝外側span単位のまま、内側spanは禁則計測専用の追加構造）。
    `mkKinCh()`/`applyKinsoku()`を、親文字の全文字を判定配列へ積み、複数文字の場合は内側spanを
    個別に測定するよう修正（`idx`に加え`sub`＝内側の文字位置を追加）
  - `test/e2e/kinsoku.e2e.ts`に回帰テストを1件追加。修正前のコードで実際に`<br>`が「──」対の間に
    挟まり分断されること（バグの再現）、修正後は分断されないことを`git stash`で退避して両方確認済み
  - **本題（`ss_000.sn:24`の列境界そのものの本家一致）は未解決のまま**：上記の修正はこのケースの
    結果には影響しなかった（「剃」「刀」自体は禁則文字集合に含まれないため、この特定の禁則違反
    ポイントには作用しなかった）。col1・col2は本家と完全一致（27文字・26文字）だが、col3のみ
    ズレる（bluesnovelは「──」を含め29行ぶん収まるが、本家は27行で打ち切り新しい列へ送る）。
    col1・col2に禁則違反が無い＝自然なCSS折返しのままで両者一致することを踏まえると、原因は
    col3固有の禁則スキャンの挙動差にある可能性が高いが、特定には両エンジンの禁則スキャンを
    ステップ単位で突き合わせるトレース作業が必要で、今回のセッションの工数を超えるため次回に
...
* **sn:** [rec_ch]のtextなし呼び出しで履歴メタデータが消える不具合を修正、sn_gallery全プロジェクト突き合わせ完了 ([a27e608](https://github.com/famibee/bluesnovel/commit/a27e60825acffa614ca24b63e2a5104ec42c9db1))
...
* **sn:** grpレイヤの位置未指定時に下端中央へ落とすフォールバックを追加 ([ff7df15](https://github.com/famibee/bluesnovel/commit/ff7df156f27792887d5aad55af076d4a9ab2b769))
...
* **sn:** React DevTools拡張機能導入環境でのSysBase初期化失敗を修正 ([a4efafc](https://github.com/famibee/bluesnovel/commit/a4efafca972c16a06f3a1fb6e4225cbf60ba9e9e))
...
* **sn:** スプライトシートの均等グリッド仮定を廃し実座標+clip-pathへ ([ff7df15](https://github.com/famibee/bluesnovel/commit/ff7df156f27792887d5aad55af076d4a9ab2b769))
* **sn:** 文字レイヤ既定表示・?cur=プロジェクト指定・DevTools干渉の不具合を修正 ([59787dd](https://github.com/famibee/bluesnovel/commit/59787dd35ce658cd367a729a7bff906d19e3a82a))
* **sn:** 縦書き時の[l]/[p]待ちマークの余分な回転を削除 ([ff7df15](https://github.com/famibee/bluesnovel/commit/ff7df156f27792887d5aad55af076d4a9ab2b769))
...
- [x] **不具合対応：ゲームパッドのスティック移動でクリック待ち記号の後ろに一瞬なにかが表示されちらつく**（2026-08-11）
  - `todo.md`「挙動の詰め・実機確認」の残り項目。キーボードの`ArrowRight`/`ArrowLeft`では起きない、パッド操作特有の症状だった
  - 原因調査：`playwright-cli`で`tmp_blues`実機（`localhost:5173`）に接続し、`GamepadMng.ts`が送るのと同じ合成`KeyboardEvent`（`isTrusted:false`）を`[l]`待ちマーカーへ向けて連投して再現を試みた。まず気付いたのは、`[set_focus]`の実行（`ScriptMng#jumpToLabelAndGo()`）が非同期のため、dispatch直後に確認すると「動いていない」ように見える（前段の処理が終わる前に見てしまうだけ）——検証時はdispatch後に十分待つ必要がある。その上で`requestAnimationFrame`間隔（約16ms）でdispatchを連投すると、`ArrowRight`のたびに`FocusMng#move()`がフォーカスの輪を確実に1要素ずつ進め、9要素の輪をちょうど一周してしまうことを実機で確認した
  - 根本原因：`GamepadMng.ts`の`#pollAxis()`はスティック軸をしきい値`DEAD_ZONE=0.3`ひとつで方向判定しており、ヒステリシスが無い。アナログ値が境界付近でわずかに揺れる（スティックのノイズ・A/D変換誤差）と、'ArrowRight'⇄''の切り替わりのたびに`if (key === prev) return;`をすり抜けてキーが再送される。各送信はそれぞれ正しく処理され、フォーカスの輪を律儀に1要素ずつ進めてしまうため、フォーカス矩形が複数要素を高速で巡回する「ちらつき」として見えていた。キーボードでは1回の押下＝1回の`keydown`（OSのキーリピートは間隔が粗い）なので発生しない
  - 修正：`axisToKey()`に既定`DEAD_ZONE`のオプショナル`zone`引数を追加（テスト・既存呼び出しは無変更）。`#pollAxis()`側で、直前のキーが非空（＝方向確定済み）の間だけ緩めの`EXIT_ZONE=0.2`を使うヒステリシスを実装。境界付近の揺れが同じ方向を維持する限りキー再送に繋がらないようにした
  - シミュレーションで効果を数値確認：境界値付近で揺れる入力列（`[0.29, 0.31, 0.28, 0.32, 0.29, 0.31, 0.30, 0.33, 0.27, 0.35]`）に対し、ヒステリシス無しでは4回の誤ったキー送信（＝フォーカスが4要素分余計に進む）が発生していたのに対し、ヒステリシス有りでは1回（本来あるべき1回だけ）に収まることを確認
  - `test/GamepadMng.test.ts`に`zone`引数の回帰テストを追加。`bun test`（6件green）・`bunx tsc --noEmit --incremental false`で確認
