...
//	sn_gallery/index.html の内蔵スクリプトビューア連携がこれと同じ関数名・同じ役割。
...
const dumpBreak = (page: Page)=> page.evaluate(
	()=> (globalThis as any).__sn.dumpBreak() as {ln: number; col: number; goto: boolean}[]);
...
test('停止のたびに break_fnc(行, 桁, goto=true)、再開で goto=false が来る', async ({page})=> {
	await seeText(page, 'いちぎょうめ');

	// 初回：[p]停止（本家 Main.ts:212）。goto=true で いちぎょうめ[p] の行（3行目）、
	//	桁は [p] トークンの位置＝「いちぎょうめ」6文字の次＝7（1始まり）
	const b0 = await dumpBreak(page);
	expect(b0.length).toBeGreaterThan(0);
	expect(b0.at(-1)).toEqual({ln: 3, col: 7, goto: true});
	expect(b0.every(v=> v.goto)).toBe(true);	// まだ再開していない
	// ワイルドカード[call fn=ext_*]展開直後（aLNum=NaN）に登録時通知が走っても、
	//	NaN 行は渡さない（次の実停止点で正しい行が来る）
	expect(b0.every(v=> Number.isFinite(v.ln) && Number.isFinite(v.col))).toBe(true);

	await pressKey(page, 'Space');
	await seeText(page, 'にぎょうめ');
	const b1 = await dumpBreak(page);
	expect(b1.some(v=> ! v.goto)).toBe(true);	// 再開通知（本家 Main.ts:205）が入った
	expect(b1.at(-1)).toEqual({ln: 4, col: 6, goto: true});	// にぎょうめ(5文字)[p]

	await pressKey(page, 'Space');
	await seeText(page, 'さんぎょうめ');
	expect((await dumpBreak(page)).at(-1)).toEqual({ln: 5, col: 7, goto: true});	// さんぎょうめ(6文字)[s]
});
