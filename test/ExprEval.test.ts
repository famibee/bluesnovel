...
// 本家 skynovel_esm/test/PropParser.test.ts の移植。
...
import {ExprEval, type T_VAR_GET} from '../src/ts/ExprEval';
import {VarStore, type T_NS, type T_VAL_D} from '../src/ts/VarStore';

import {beforeEach, expect, it} from 'bun:test';


//===== Test Class =====
class ValTest implements T_VAR_GET {
	readonly #hGetVal: {[nm: string]: T_VAL_D} = {
		'mp:fn'			: 'うひひ',
		'mp:lay'		: 'もきゅ',
		'mp:pos'		: 'うひひ',
		'ぎょへー'		: 'もきゅ',
	...
	get(nm: string): T_VAL_D {return this.#hGetVal[nm]}
}
//===== Test Class =====
...
let parser: ExprEval;
beforeEach(()=> {
	parser = new ExprEval(new ValTest);
});
...
for (const [idx, c] of aCase.entries()) it(`${String(idx)}: ${c.i}`, ()=> {
	const {i, o, toBeNull, toBeNaN, toBeUndefined, toBeTruthy, toThrowError} = c;
...
// 変数名の分解（本家 PropParser.getValName() 相当 → bluesnovelでは VarStore.parseName()）
const aCaseNm: {i: string; ns: T_NS; key: string; atStr?: boolean}[] = [
	{i: 'mp:fn',		ns: 'mp',	key: 'fn'},
	{i: 'fn',			ns: 'tmp',	key: 'fn'},
	{i: 'ぎょへー',		ns: 'tmp',	key: 'ぎょへー'},
	{i: 'hC.5reg',		ns: 'tmp',	key: 'hC.5reg'},
	{i: 'hA.秋冬.args',	ns: 'tmp',	key: 'hA.秋冬.args'},
	{i: 'sys:_album.img.渡り廊下・桜昼',	ns: 'sys',	key: '_album.img.渡り廊下・桜昼'},

	{i: "hA['args']",			ns: 'tmp',	key: 'hA.args'},
	{i: 'hA["args"]',			ns: 'tmp',	key: 'hA.args'},
	{i: 'hA["args"]["rrr"]',	ns: 'tmp',	key: 'hA.args.rrr'},

	{i: 'tmp:one_s@str',		ns: 'tmp',	key: 'one_s',	atStr: true},
];

for (const c of aCaseNm) it(`parseName: ${c.i}`, ()=> {
	const o = VarStore.parseName(c.i);
	expect(o.ns).toBe(c.ns);
	expect(o.key).toBe(c.key);
	expect(o.atStr).toBe(c.atStr ?? false);
});

it('parseName_err0', ()=> {
	// 本家 PropParser.getValName() はnullを返すが、bluesnovelは例外にする
	expect(()=> VarStore.parseName('mp:f:n')).toThrow();
});


// ===== ここから下は bluesnovel 固有（本家PropParser.test.tsには無い）=====
//	実行時は ValTest ではなく VarStore を渡すので、その組み合わせを確認する

it('bluesnovel_ns_game', ()=> {	// 本家の save: に相当する名前空間
	const vs = new VarStore;
	const ev = new ExprEval(vs);
	vs.set('game:hp', 80);
...
it('bluesnovel_evalBool', ()=> {	// [if]/[elsif]用の真偽判定
	const vs = new VarStore;
	const ev = new ExprEval(vs);
	expect(ev.evalBool('true')).toBe(true);
	expect(ev.evalBool('false')).toBe(false);
	expect(ev.evalBool('1 < 2')).toBe(true);
	expect(ev.evalBool('1 > 2')).toBe(false);
	expect(ev.evalBool('未定義変数')).toBe(false);
	vs.set('game:flag', 'false');
	expect(ev.evalBool('game:flag')).toBe(false);	// 文字列'false'も偽扱い
	vs.set('game:s', 'もじ');
	expect(ev.evalBool('game:s')).toBe(true);
});

it('bluesnovel_getValAmpersand', ()=> {	// タグ属性の「&計算」書式
	const vs = new VarStore;
	const ev = new ExprEval(vs);
	vs.set('game:hp', 80);
	expect(ev.getValAmpersand('&game:hp + 1')).toBe('81');
	expect(ev.getValAmpersand('game:hp + 1')).toBe('game:hp + 1');	// &なしは素通し
});

it('bluesnovel_doubleQuoteString', ()=> {
	const vs = new VarStore;
	const ev = new ExprEval(vs);
...
it('bluesnovel_powerIsRightAssoc', ()=> {
	const vs = new VarStore;
	const ev = new ExprEval(vs);
	expect(ev.parse('2**3**2')).toBe(512);	// 2**(3**2)
