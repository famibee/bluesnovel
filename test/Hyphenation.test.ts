...
//MARK: [l]境界の番兵ズレ（設計検討用の再現テスト。src/docs/text-rendering.md「[l]境界をまたぐ
//	禁則ズレ」参照）
//	TxtLayer.tsxのapplyKinsoku()は行末の最後の表示単位に「次に何か置いたら溢れるか」を判定させる
//	ため、ダミーの半角スペース1文字を番兵として使う（TxtLayer.tsx:973-978）。しかし[l]の直後に
//	実際に続く文字が行頭禁則文字等の場合、番兵での判定と食い違いうる。同じ配列・同じ折返し位置で
//	最後の要素だけをダミー空白／実際の次文字に差し替え、scan()の結果が変わることを示す
it('番兵=ダミー空白では違反なしと判定される（実際は禁則違反の可能性がある）', ()=> {
	const a: T_KIN_CH[] = [{ch: '㍿'}, {ch: 'こ'}, {ch: 'の'}, {ch: ' '}];
	expect(kin.scan(a, [0, 10, 20, -1], false, 2)).toBeNull();	// 違反なし＝<br>不要と判定
});

it('番兵=実際の次文字（行頭禁則文字）なら違反ありと判定される', ()=> {
	// 「〉」は[l]の後に実際に続きうる行頭禁則文字（DEF_KINSOKU_SOL）の一例
	const a: T_KIN_CH[] = [{ch: '㍿'}, {ch: 'こ'}, {ch: 'の'}, {ch: '〉'}];
	expect(kin.scan(a, [0, 10, 20, -1], false, 2)).toEqual({ins: 2, resumeAt: 4});	// 違反あり＝<br>が要る
});
